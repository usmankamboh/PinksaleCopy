...
contract Mooniswap is MooniswapGovernance {
    ...
    IBEP20 public immutable token0;
    ...
    function getTokens() external view returns(IBEP20[] memory tokens) {
        tokens = new IBEP20[](2);
        tokens[0] = token0;
        tokens[1] = token1;
    }
    ...
    function tokens(uint256 i) external view returns(IBEP20) {
        if (i == 0) {
            return token0;
        } else if (i == 1) {
            return token1;
    ...
    function getBalanceForAddition(IBEP20 token) public view returns(uint256) {
    ...
    function getBalanceForRemoval(IBEP20 token) public view returns(uint256) {
        uint256 balance = token.uniBalanceOf(address(this));
    ...
    function getReturn(IBEP20 src, IBEP20 dst, uint256 amount) external view returns(uint256) {
    ...
    function depositFor(uint256[2] memory maxAmounts, uint256[2] memory minAmounts, address target) public payable nonReentrant returns(uint256 fairSupply, uint256[2] memory receivedAmounts) {
        IBEP20[2] memory _tokens = [token0, token1];
    ...
    function swapFor(IBEP20 src, IBEP20 dst, uint256 amount, uint256 minReturn, address referral, address payable receiver) public payable nonReentrant whenNotShutdown returns(uint256 result) {
    ...
    function _doTransfers(IBEP20 src, IBEP20 dst, uint256 amount, uint256 minReturn, address payable receiver, Balances memory balances, Fees memory fees)
    ...
    */
    function _getReturn(IBEP20 src, IBEP20 dst, uint256 amount, uint256 srcBalance, uint256 dstBalance, uint256 fee, uint256 slippageFee) internal view returns(uint256) {
        if (src > dst) {
            (src, dst) = (dst, src);
        }
    ...
    function rescueFunds(IBEP20 token, uint256 amount) external nonReentrant onlyOwner {
